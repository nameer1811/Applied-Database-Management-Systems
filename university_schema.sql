...
CREATE TABLE `advisor` (
  ...
  PRIMARY KEY (`s_ID`),
  KEY `i_ID_idx` (`i_ID`),
  CONSTRAINT `i_ID` FOREIGN KEY (`i_ID`) REFERENCES `instructor` (`ID`) ON DELETE SET NULL ON UPDATE NO ACTION,
  CONSTRAINT `s_ID` FOREIGN KEY (`s_ID`) REFERENCES `student` (`ID`) ON DELETE CASCADE ON UPDATE NO ACTION
...
INSERT INTO `classroom` VALUES ('Packard','101',50),('Painter','514',10),('Taylor','3128',70),('Watson','100',30),('Watson','120',50);
...
INSERT INTO `grade_points` VALUES ('A',4),('A-',3.7),('B',3),('B+',3.3),('B-',2.7),('C',2),('C+',2.3),('D',1),('D+',1.3),('D-',0.7),('F',0);
...
CREATE TABLE `instructor` (
  ...
  CONSTRAINT `dept_name` FOREIGN KEY (`dept_name`) REFERENCES `department` (`dept_name`) ON DELETE SET NULL ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `prereq` (
  ...
  CONSTRAINT `prereq_id` FOREIGN KEY (`prereq_id`) REFERENCES `course` (`course_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `teaches` (
  ...
  `year` decimal(4,0) NOT NULL DEFAULT '0',
  PRIMARY KEY (`ID`,`course_id`,`sec_id`,`semester`,`year`),
  KEY `course_id, sec_id, semester, year_idx` (`course_id`,`sec_id`,`semester`,`year`),
  KEY `ID_idx` (`ID`),
  CONSTRAINT `course_id??sec_id??semester??year` FOREIGN KEY (`course_id`, `sec_id`, `semester`, `year`) REFERENCES `section` (`course_id`, `sec_id`, `semester`, `year`) ON DELETE CASCADE ON UPDATE NO ACTION,
  CONSTRAINT `ID` FOREIGN KEY (`ID`) REFERENCES `instructor` (`ID`) ON DELETE CASCADE ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
--
/*!50003 DROP PROCEDURE IF EXISTS `Count_Courses` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `Count_Courses`(
	IN Instructor_Name VARCHAR(20),
	OUT resultCount INT
	)
    DETERMINISTIC
    COMMENT 'Count the number of courses given by an input instructor (name).'
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Hello_World`()
    DETERMINISTIC
    COMMENT 'My Hello_World My SQL Stored Procedure'
BEGIN
	SELECT 'Hello World !';
END ;;
DELIMITER ;
...
-- Dump completed on 2013-08-19 12:56:03
